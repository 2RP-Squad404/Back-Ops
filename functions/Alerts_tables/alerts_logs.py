...
table_id = f"{project_id}.alerts.logs"
...
for row in rows:
    # cria um alerta de log a partir dessa linha
    json_log["display_name"] = row.alert_name
    json_log["conditions"][0]["condition_matched_log"]["filter"] = row.log_query
    json_log["notification_channels"] = [f'{project_name}/notificationChannels/{row.id_email}']

    print("JSON de log alterado")

    alert_policy = client_monitoring.create_alert_policy(name = project_name, alert_policy = json_log)
    ...
    WHERE alert_name = '{row.alert_name}' AND log_query = '{row.log_query}' AND id_email = '{row.id_email}'
    """

    print(f"Alerta de Log criado para a linha: {row}")
...
for policy in policies:
    ...
    if (policy.name not in [f'{project_name}/alertPolicies/{r.alert_id}' for r in rows] and any('condition_matched_log' in condition for condition in policy.conditions)):
        # Exclui os alertas
        try:
